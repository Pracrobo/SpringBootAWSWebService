...
import com.springboot.practice.config.auth.dto.OAuthAttributes;
import com.springboot.practice.config.auth.dto.SessionUser;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private User saveOrUpdate(OAuthAttributes attributes) {
        User user = userRepository.findByEmail(attributes.toEntity().getEmail()).map(entity -> entity.update(attributes.toEntity().getName(), attributes.toEntity().getPicture())).orElse(attributes.toEntity());
